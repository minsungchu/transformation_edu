 ...
 *
 * 툴바에는 티칭 펜던트식 조그(jog)가 붙어 있다. 같은 +X 버튼이라도 World
 * 기준과 Tool 기준에서 로봇이 다르게 움직이는 것을 눈으로 보는 게 목적이라,
 * Cartesian 모드에서는 해당 기준 좌표계의 축을 씬에 함께 그린다.
 */
...
// 좌표계 이름은 CONTEXT.md 용어(World / Tool)를 따르고, 티칭 펜던트에서 쓰는
// 버튼 이름(Base)은 괄호로만 붙인다.
const JOG_MODES: {id: JogMode; label: string; hint: string}[] = [
  {
    id: 'base',
    label: 'World(Base)',
    hint: 'World 좌표계 기준 — TCP가 어떻게 기울어져 있든 씬에 표시된 World 축 방향으로 움직인다.',
  },
  {
    id: 'tool',
    label: 'Tool',
    hint: 'Tool 좌표계(TCP) 기준 — TCP 자신의 축 방향으로 움직인다. 같은 +X여도 World 기준과 다른 쪽으로 간다.',
  },
  {
    id: 'joint',
    label: 'Joint',
...
export default function BinPickingPipeline({height}: {height?: number} = {}): ReactNode {
  ...
  const stepCartesian = (spec: (typeof CARTESIAN_AXES)[number], sign: 1 | -1): void => {
    ...
    const ok = sceneRef.current?.jogCartesian(jogMode === 'tool' ? 'tool' : 'base', {
      kind: spec.kind,
      axis: spec.axis,
      amount,
    });
  ...
  return (
    <figure
            ...
            {jogMode === 'joint'
              ...
              : CARTESIAN_AXES.map((spec) => (
                  <JogChip
                    key={spec.label}
                    label={spec.label}
                    title={`${jogMode === 'tool' ? 'Tool' : 'World'} 기준 ${spec.label}`}
                    disabled={jogDisabled}
      ...
      <figcaption style={{fontSize: '0.85rem', opacity: 0.75, marginTop: '0.5rem', textAlign: 'center'}}>
        ...
        전부 World·Camera·Scene에 고정된 관계이기 때문이다.
      </figcaption>
    </figure>
  );
}
