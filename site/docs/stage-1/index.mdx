...
설치된 카메라. 마우스로 드래그해 셀을 직접 돌려보고, 아래 토글로 좌표계 축(RGB
화살표), Post ↔ Robot Mount 전환, 좌표계 사이의 $T$ 화살표(꼬리 = 기준, 화살촉 =
타겟)를 켜 보세요.
...
- [1-3. Mount 두 방식](./mount-types.mdx) — Post Mount와 Robot Mount의 차이
